...
##
model2 <- df_ev %>% lm(log(price) ~ odo + my + kw + range_km + body_type + make, data = .)
summary(model2)
```
...
Most estimations are significant except model years 2013. For the increment of every 10,000 km of odometers, the price is estimated to reduce 1.6% with significant evidence. This decrement seems to be fairly small. The kw and range_km are correlated in regression but I will not further investigate the endogeneity in this post.

The linear regression model, however, demonstrates that **every 1 kw increment in battery is estimated to increase 0.2627% on price**. 
