...
# Which state has a higher price on average?
...
gg_state <- df_states[, avg_rate := percent(averagprice/min(averagprice)-1)][, med_rate := medprice/min(medprice)] %>% 
  ...
  geom_col_interactive(aes(fill = state_only, tooltip = averagprice, data_id = round(averagprice, 2)))+
...
```{r barstate, fig.cap='Average Price in States & % changes compare to NT', include=TRUE}
ggiraph(ggobj = gg_state)
```

From the figure \@ref(fig:barstate), NT has the lowest average price in eight states. NSW and VIC have over 20% increment in average price compare to NT. This percentage change could be vital if you buy a luxury vehicle or several cars for business. 
